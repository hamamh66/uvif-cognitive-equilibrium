...
Section~\ref{sec:future} specifies common evaluation requirements. This paper does not report tests of H1--H4 or claim that they distinguish UVIF from every member of the comparator families. The companion repository provides an executed notebook and saved outputs for illustrative policy-grid comparisons \citep{Hamam2026GitHub}. Its implementation maximizes a scalar score formed from the five named contributions, rather than integrating Eq.~\eqref{eq:projected}. Its fixed-reference H1 comparator also freezes the environment arguments of the component functions; this tests state responsiveness, not an impossibility of fixed-weight scalar optimization. The H3 search examines a particular power-penalty family over bounded grids, which cannot establish non-equivalence to every finite monotone penalty. These demonstrations motivate revised comparisons but are not presented here as validation of H1--H4 or of the vector-field model.
